...
def test_crf_json_non_pos_non_BILOU(spacy_nlp, ner_crf_non_pos_feature_config):
    from rasa_nlu.extractors.crf_entity_extractor import CRFEntityExtractor
    ner_crf_non_pos_feature_config.update({"BILOU_flag": False})
    ext = CRFEntityExtractor(component_config=ner_crf_non_pos_feature_config)
    sentence = u"I need a home cleaning close-by"
    doc = {"spacy_doc": spacy_nlp(sentence)}
    rs = ext._from_crf_to_json(Message(sentence, doc),
                               [{'O': 1.0},
...
def test_duckling_entity_extractor_and_synonyms(component_builder):
    ...
    duckling.process(message)
    # checks that the synonym processor can handle entities that have int
    # values
